export class NuclearMissileProjectile {
  constructor(x, y, vx, vy, damage, target, tileSize = 32) {
    ...
    this.radius = 0.28; // in tile units
    this.speed = Math.sqrt(vx * vx + vy * vy);
    this.alive = true;
    this.exploded = false;
    this.explosionRadius = 2.2; // in tile units
  ...
  render(ctx, tileSize, canvas, mapConfig) {
    ...
    if (this.exploded) {
      ...
      ctx.fill();
      ctx.beginPath();
      ctx.arc(px, py, this.explosionRadius * 0.7 * tileSize * (this.explosionFrame / this.explosionDuration), 0, 2 * Math.PI);
    ...
    for (let i = 0; i < this.trail.length; i++) {
      ...
      ctx.beginPath();
      ctx.arc(t.x * tileSize, t.y * tileSize, tileSize * 0.18, 0, 2 * Math.PI);
      ctx.fillStyle = '#ffeb3b';
      ctx.fill();
      ctx.restore();
    }
